...
def load_dataset(Dataset, Split):
    ...
    if Dataset == 'SA':
        ...
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
        '''
        mat_data = sio.loadmat('datasets/SalinasA_corrected.mat')
        mat_gt = sio.loadmat('datasets/SalinasA_gt.mat')
        data_hsi = mat_data['salinasA_corrected']
        gt_hsi = mat_gt['salinasA_gt']
        TOTAL_SIZE = 5348
        VALIDATION_SPLIT = Split
        TRAIN_SIZE = math.ceil(TOTAL_SIZE * VALIDATION_SPLIT)
...
def detection_map(map, ground_truth, dpi, save_path):
...
def list_to_colormap1(x_list, classtype):
    y = np.zeros((x_list.shape[0], 3))
    for index, item in enumerate(x_list):
        if item == classtype:
            y[index] = np.array([255, 215, 0]) / 255.
...
'''
def generate_png(all_iter, net, gt_hsi, Dataset, device, total_indices, classtype):
    pred_test = []
    # with torch.no_grad():
    ...
    y_list = list_to_colormap1(x, classtype)
    y_gt = list_to_colormap1(gt, classtype)
    ...
    classification_map(y_re, gt_hsi, 300,
                       net.name + '/classification_maps/' + Dataset + '_' + net.name + '.png')
    classification_map(gt_re, gt_hsi, 300,
                       net.name + '/classification_maps/' + Dataset + '_gt.png')
...
def generate_png(all_iter, net, gt_hsi, Dataset, device, total_indices, classtype):
    ...
    for X, y in all_iter:
        X = X.to(device)
        net.eval()  # 评估模式
        y_hat = sigmoid(net(X).cpu().detach().numpy())
        # print(net(X))
        pred_test.extend(np.array(net(X).cpu().argmax(axis=1)))
        pred_possibility.extend(y_hat.tolist())

    gt = gt_hsi.flatten()
    x_label = np.zeros(gt.shape)

    pred_possofclass = [i[classtype] for i in pred_possibility]
    x_possibility = np.zeros(gt.shape)
    #x_possresofclass = [i[0] for i in x_possibility]
    ...
    x_possibility[total_indices] = pred_possofclass
    x_ = np.ravel(x_possibility)
    ...
    y_re4roc = x / max(x)
    gt_onehot = preprocessing.label_binarize(gt, classes=np.arange(17))
    y_onehot = preprocessing.label_binarize(x, classes=np.arange(17))
    gttemp = gt_onehot.flatten()
    ytemp = y_onehot.flatten()

    fpr, tpr, thresholds = metrics.roc_curve(gt, x_, pos_label=classtype)
    fpr_list = fpr.tolist()
    tpr_list = tpr.tolist()
    print("fpr1 = ", fpr_list)
    print("tpr1 = ", tpr_list)
    roc_auc = metrics.auc(fpr_list, tpr_list)
    print("in generate", roc_auc)

    confusion_matrix = metrics.confusion_matrix(x, gt)


    day = datetime.datetime.now()
    day_str = day.strftime('%m_%d_%H_%M')
    record.record_roc(tpr, fpr, roc_auc, 'records/' + net.name + day_str + '_' + Dataset + 'classnum='+ str(classtype) + '_roc.txt')

    plt.semilogx(fpr_list, tpr_list, color='darkorange', lw=2, label = 'spectral-spatial\nAUC = %0.5f' % roc_auc)
    ...
    detection_map(y_re, gt_hsi, 300,
                       net.name + '/detection_maps/' + Dataset + '_' + net.name + '.png')
    detection_map(gt_re, gt_hsi, 300,
                       net.name + '/detection_maps/' + Dataset + '_gt.png')

    print('------Get detection maps successful-------')
